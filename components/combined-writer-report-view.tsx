...
export function CombinedWriterReportView({ report, generatedLine }: Props) {
  return (
    <div className="insights-report-print mx-auto max-w-4xl bg-white text-zinc-900 shadow-xl ring-1 ring-zinc-200">
      <header className="bg-[#1a365d] px-8 py-7 text-white print:bg-[#1a365d]">
        ...
        <p className="mt-2 text-sm text-white/85">{report.environmentSubtitle}</p>
      </header>
      ...
      <div className="px-8 pb-10 pt-10">
        <section className="mb-12">
          ...
          <div className="mt-6 grid grid-cols-1 gap-px bg-zinc-300 sm:grid-cols-2">
            {report.section1Items.map((item, i) => (
              <div key={`${item.title}-${i}`} className="min-h-[100px] bg-white p-5">
        ...
        <section className="mb-12">
          ...
          <ul className="mt-6 flex flex-col gap-4">
            {report.section2Items.map((item, i) => (
              ...
              >
                <span className="shrink-0 pt-0.5 text-lg font-bold text-red-600" aria-hidden>
                  ×
                </span>
                <div>
                  <h3 className="font-semibold text-zinc-900">{item.title}</h3>
        ...
        <section className="mb-12 border-t border-zinc-200 pt-10">
          <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-zinc-500">
            Section 4
          </p>
          <h2 className="mt-1 font-[family-name:var(--font-sans)] text-2xl font-bold tracking-tight text-zinc-900">
            Pruned-task patterns to improve your success rate
          </h2>
          <p className="mt-2 text-sm leading-relaxed text-zinc-600">
            Patterns below target fixes in the task instructions — not rubrics, verifiers, or
            environment. Guidance assumes tasks remain complex and reasoning-heavy: tighten
            unclear or inconsistent specs rather than adding exhaustive step-by-step hand-holding.
          </p>
          <p className="mt-3 whitespace-pre-wrap text-sm leading-relaxed text-zinc-700">
            {report.section4Intro}
          </p>

          <h3 className="mt-7 text-lg font-semibold text-zinc-900">High-risk patterns</h3>
          <div className="mt-3 flex flex-col gap-3">
            {report.section4RiskItems.map((item, i) => (
              <article key={`${item.title}-${i}`} className="rounded-xl border border-zinc-200 bg-zinc-50 p-4">
          ...
          <h3 className="mt-7 text-lg font-semibold text-zinc-900">Pruned-informed action plan</h3>
          <div className="mt-3 flex flex-col gap-3">
            {report.section4ActionItems.map((item, i) => (
              <article key={`${item.title}-${i}`} className="rounded-xl border border-zinc-200 bg-zinc-50 p-4">
                <h4 className="text-base font-semibold text-zinc-900">{item.title}</h4>
                <p className="mt-2 text-sm leading-relaxed text-zinc-700">{item.body}</p>
              </article>
