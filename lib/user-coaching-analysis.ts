...
export async function runUserCoachingAnalysis(
  ...
  const system = [
    ...
    "**What the author can vs. cannot fix (mandatory):** Read excerpts/rationales for **system-level** problems (e.g. task flagged **bugged / not gradable**, grading pipeline failure, **escalation to external QA review or engineering**, rubric/tool mismatch **outside** revising prompt copy). When that pattern appears, **do not** write **`example`** text or **`coachingActions`** that tell the task author to “check grading criteria,” “align the task with the rubric,” or otherwise imply they can resolve an infrastructure or platform fault by editing the task alone. That coaching is **invalid** and **must be avoided**. Instead model **legitimate** guidance: reviewer language that **names the constraint**, **routes to the right owner**, **requests a ticket or clarification**, or helps the author improve **only what they control** (clarity, scope, constraints) **without** blaming them for the bug. If samples mix author-fixable and system issues, **split** the two in your observation/example so actionable advice targets only **in-scope** levers.",
    "**Escalations out of QA / author control (mandatory):** Many **escalated** items (external QA review, flagged bugged, cannot grade, etc.) are **not resolvable by the task writer or by QA feedback alone**—resolution sits with **another owner or pipeline**. In those cases you **must not** imply in **`example`** or **`coachingActions`** that the author should **verify rubric alignment**, **fix grading criteria**, or take similar steps as if they owned the grading stack. **Do not** assign homework that assumes QA or the writer can close the escalation. **Do** coach: **clearer documentation** in the escalation (symptoms, what was tried, what blocks grading), **honest framing** that follow-up is with the owning team, and **reviewer wording** that avoids dumping platform faults on the author. Praising how to communicate under escalation is fine; inventing author-side \"fixes\" for platform-level issues is not.",
